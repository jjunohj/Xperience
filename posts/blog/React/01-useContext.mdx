...
요즘은 보통 다음과 같이 상태관리를 하는 경우가 많은 것 같아요.

1. `useState`: 단일 컴포넌트의 상태를 관리
2. 전역 상태 라이브러리 (Zustand, Recoil 등): 앱 전역에서 사용되며, 생명주기가 앱과 거의 동일한 상태를 관리 (사용자 정보, 테마 등)
3. 서버 상태 관리 라이브러리 (React query, SWR 등): 서버로부터 내려오는 상태를 캐시 및 관리

여기서 한 가지 질문을 해보고 싶어요. Context API는 위 3가지 도구 중 몇 번에 해당할까요?

2번이라고 생각하셨나요? 만약 그렇다면 Context API를 리렌더링 때문에 실무에서 잘 쓰이지 않는 전역 상태 관리 도구로 알고 계셨을 확률이 높아요.

실제로 Context API를 구글에 검색하면, '전역 상태 관리' 라는 말이 가장 많이 보이기도 합니다.

<img
  width="700"
  alt="Image"
  src="https://github.com/user-attachments/assets/d158e80a-3fa7-44b9-abd1-0279ae49c8ad"
/>

하지만, 사실 Context API는 전역 상태 관리가 **가능한** 도구이지, **전역 상태 관리를 위한 도구**가 아닙니다.

마치 망치 꼬투리로 나사를 돌릴 수는 있지만, 나사를 돌리는 용도로만 망치를 사용하지 않는 것과 마찬가지죠.

그럼, Context API는 도대체 무슨 역할을 하고, 언제 사용하는 것이 가장 효율적일까요?

이번 글에서는 Context API의 역할과 최적의 쓰임새를 깊이 있게 다루고, 특히 특정 영역에서 Context API가 얼마나 강력하고 우아한 도구가 될 수 있는지 보여드리고자 합니다.

## Context API란

Context API는 React의 컴포넌트 트리 안에서 데이터를 **특정 범위 내에서 상태를 공유**할 수 있도록 해주는 내장 기능입니다. 키워드는 **특정 범위**입니다.

Context API를 사용하면 컴포넌트 트리의 특정 범위 내에서는, 여러 컴포넌트에 데이터를 전달하기 위해 여러 단계를 거쳐 props를 내려주지 않고도 범위 내의 모든 컴포넌트에서 직접 값을 전달받을 수 있습니다.
...
const ParentComponent = () => {
  ...
  /* 3. Context.Provider를 통해 하위 컴포넌트에 값 전달 (영역전개)*/
  return (
...
위 예시에서는 ParentComponent의 상태를 MyContext 값에 담고, 그 값을 공유할 범위인 MyContext.Provider에 감싸서 하위 컴포넌트에 전달합니다.

이렇게 '특정 범위' 내에서 상태를 공유할 수 있다는 '특성'을 바탕으로, 그 '범위'를 애플리케이션 전체로 넓혀 전역에서 사용하도록 하면 이제 Context API는 '전역 상태 관리 도구'가 되는 겁니다.

하지만 저는 이를 크게 추천드리지 않아요. 바로 Context API의 또 다른 특성 때문입니다.

## Context API의 한계

언뜻 보기에 Context API를 사용하면 굉장히 쉽고 직관적으로 전역 상태 관리가 가능할 것으로 보입니다. 그냥 Context에 전역으로 사용할 상태들을 담고, 애플리케이션 전체를 감싸버리면 만사 OK거든요. 하지만 Context API에는 크게 두 가지 한계점이 존재합니다.
...
아래 예시를 통해 문제를 구체적으로 살펴보겠습니다. 아래 예시는 사용자 정보와 테마 상태를 하나의 Context에 관리하는 경우입니다.
...
사용자가 Provider 내부의 특정 컴포넌트에서 theme을 변경했을 때 다음과 같은 일이 일어납니다.

1. TotalAppProvider의 `theme` 상태가 `"light"`에서 `"dark"`로 변경됩니다.
...
첫 번째로 부모가 렌더링되면, 기본적으로 그 자식들도 모두 렌더링된다는 React의 원칙에 따라 TotalAppProvider의 {children}들은 모두 리렌더링됩니다.

두 번째로 value의 prop 참조가 변경되었기 때문에, useContext(TotalAppContext)를 사용하는 모든 컴포넌트들 또한 Context 변경을 감지하고 리렌더링됩니다.

이 두 가지는 같은 시점에 발생하며, 결과적으로 **리렌더링**이라는 동일한 현상으로 이어집니다.

**따라서, Context의 상태 변경이 빈번하거나, 컴포넌트 트리가 깊거나, 구독하는 컴포넌트가 많을수록 성능 저하 이슈가 발생합니다.**

### 2. 다양한 상태 관리의 구조적 문제

- 여러 상태를 하나의 Context에 관리하면, 관리가 복잡해지고 value 변경에 의한 불필요한 리렌더링이 발생합니다.

- 여러 Context로 분리해서 사용하면, 중첩된 Provider(Provider Hell)가 발생하고, 연쇄 리렌더링이 발생할 수 있습니다.
