...
const PostCard = (post: Post) => {
  return (
    <motion.a
      ...
      whileTap={{ scale: 0.95 }}
    >
      <div className="relative h-24 w-full rounded-2xl sm:h-60 sm:w-40 md:h-72 md:w-56">
        <Image
          src={post.thumbnail}
          alt={post.title}
          width={224}
        ...
        <div className="absolute bottom-0 right-0 flex h-full w-3/4 flex-col items-start justify-start gap-1.5 overflow-hidden bg-white bg-opacity-60 p-4 dark:bg-black dark:bg-opacity-30 sm:left-0 sm:h-[136px] sm:w-full sm:gap-2">
          <IconText
            Icon={CalenderIcon}
            text={format(parseISO(post.date), "yyyy.MM.dd")}
          />

          <h2 className="flex-wrap break-words text-sm font-semibold text-black dark:text-white sm:text-base">
            {post.title}
          </h2>
        </div>
