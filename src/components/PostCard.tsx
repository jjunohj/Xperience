  ...
  return (
    ...
    >
      <div className="relative h-20 w-full rounded-2xl sm:h-60 sm:w-40 md:h-72 md:w-56">
        ...
        <div className="absolute bottom-0 right-0 flex h-full w-3/4 flex-col items-start justify-start gap-1 overflow-hidden bg-white bg-opacity-60 px-4 py-2 dark:bg-black dark:bg-opacity-30 sm:left-0 sm:h-[136px] sm:w-full sm:gap-2 sm:p-4">
          <IconText
            Icon={CalenderIcon}
            text={format(parseISO(post.date), "yyyy.MM.dd")}
          />

          <h2 className="flex-wrap overflow-x-hidden text-ellipsis whitespace-nowrap text-sm font-semibold text-black dark:text-white sm:break-words sm:text-base">
        ...
        <div className="absolute bottom-4 left-4 hidden sm:block">
          <div className="flex gap-2 text-sm opacity-90">
            {post.tags.map((tag) => (
              <Pill
                key={tag}
                className="bg-neutral-50 font-light dark:bg-neutral-900"
              >
                {tag}
              </Pill>
            ))}
